...
impl<T: FilesystemMT + Sync + Send + 'static> fuse::Filesystem for FuseMT<T> {
    ...
    fn lookup(
        ...
        match self
            .rt
            .block_on(self.target.getattr(req.info(), &path, None))
        {
            Ok((ttl, attr)) => {
    ...
    fn setattr(
        ...
        if let Some(mode) = mode {
            if let Err(e) = self
                .rt
                .block_on(self.target.chmod(req.info(), &path, fh, mode))
            {
                reply.error(e);
                return;
            }
        ...
        if uid.is_some() || gid.is_some() {
            if let Err(e) = self
                .rt
                .block_on(self.target.chown(req.info(), &path, fh, uid, gid))
            {
                reply.error(e);
                return;
            }
        ...
        if let Some(size) = size {
            if let Err(e) = self
                .rt
                .block_on(self.target.truncate(req.info(), &path, fh, size))
            {
                reply.error(e);
                return;
            }
        ...
        if atime.is_some() || mtime.is_some() {
            if let Err(e) =
                self.rt
                    .block_on(self.target.utimens(req.info(), &path, fh, atime, mtime))
            {
                reply.error(e);
                return;
            }
        ...
        if crtime.is_some() || chgtime.is_some() || bkuptime.is_some() || flags.is_some() {
            if let Err(e) = self.rt.block_on(self.target.utimens_macos(
                req.info(),
                &path,
                fh,
                crtime,
    ...
    fn mkdir(
        ...
        match self
            .rt
            .block_on(self.target.mkdir(req.info(), &parent_path, name, mode))
        {
            Ok((ttl, attr)) => {
                let (ino, generation) = self.inodes.add(Arc::new(parent_path.join(name)));
                reply.entry(&ttl, &fuse_fileattr(attr, ino), generation)
    ...
    fn unlink(
        ...
        match self
            .rt
            .block_on(self.target.unlink(req.info(), &parent_path, name))
        {
            Ok(()) => {
    ...
    fn rmdir(
        ...
        match self
            .rt
            .block_on(self.target.rmdir(req.info(), &parent_path, name))
        {
            Ok(()) => reply.ok(),
            Err(e) => reply.error(e),
        }
    ...
    fn symlink(
        ...
        match self
            .rt
            .block_on(self.target.symlink(req.info(), &parent_path, name, link))
        {
            Ok((ttl, attr)) => {
                let (ino, generation) = self.inodes.add(Arc::new(parent_path.join(name)));
                reply.entry(&ttl, &fuse_fileattr(attr, ino), generation)
    ...
    fn rename(
        ...
        match self.rt.block_on(self.target.rename(
            req.info(),
            &parent_path,
            name,
            &newparent_path,
    ...
    fn read(
        ...
        self.rt.spawn_task(async move {
            target
                .read(req_info, &path, fh, offset as u64, size, move |result| {
                    match result {
                        Ok(data) => reply.data(data),
                        Err(e) => reply.error(e),
                    }
    ...
    fn write(
        ...
        self.rt.spawn_task(async move {
            match target
                .write(req_info, &path, fh, offset as u64, data_buf, flags)
                .await
            {
    ...
    fn release(
        ...
        match self.rt.block_on(
            self.target
                .release(req.info(), &path, fh, flags, lock_owner, flush),
        ) {
            Ok(()) => reply.ok(),
            Err(e) => reply.error(e),
        }
    ...
    fn opendir(&mut self, req: &fuse::Request<'_>, ino: u64, flags: u32, reply: fuse::ReplyOpen) {
        ...
        match self
            .rt
            .block_on(self.target.opendir(req.info(), &path, flags))
        {
            Ok((fh, flags)) => {
    ...
    fn readdir(
        ...
        let entries: &[DirectoryEntry] = {
            ...
            if let Some(ref entries) = dcache_entry.entries {
                ...
                match self
                    .rt
                    .block_on(self.target.readdir(req.info(), &path, dcache_entry.fh))
                {
                    Ok(entries) => {
    ...
    fn releasedir(
        ...
        match self
            .rt
            .block_on(self.target.releasedir(req.info(), &path, real_fh, flags))
        {
            Ok(()) => reply.ok(),
            Err(e) => reply.error(e),
        }
    ...
    fn fsyncdir(
        ...
        match self
            .rt
            .block_on(self.target.fsyncdir(req.info(), &path, real_fh, datasync))
        {
            Ok(()) => reply.ok(),
            Err(e) => reply.error(e),
        }
    ...
    fn setxattr(
        ...
        match self.rt.block_on(self.target.setxattr(
            req.info(),
            &path,
            name,
            value,
    ...
    fn getxattr(
        ...
        match self
            .rt
            .block_on(self.target.getxattr(req.info(), &path, name, size))
        {
            Ok(Xattr::Size(size)) => {
    ...
    fn listxattr(&mut self, req: &fuse::Request<'_>, ino: u64, size: u32, reply: fuse::ReplyXattr) {
        ...
        match self
            .rt
            .block_on(self.target.listxattr(req.info(), &path, size))
        {
            Ok(Xattr::Size(size)) => {
    ...
    fn removexattr(
        ...
        match self
            .rt
            .block_on(self.target.removexattr(req.info(), &path, name))
        {
            Ok(()) => reply.ok(),
            Err(e) => reply.error(e),
        }
    ...
    fn access(&mut self, req: &fuse::Request<'_>, ino: u64, mask: u32, reply: fuse::ReplyEmpty) {
        ...
        match self
            .rt
            .block_on(self.target.access(req.info(), &path, mask))
        {
            Ok(()) => reply.ok(),
            Err(e) => reply.error(e),
        }
    ...
    fn create(
        ...
        match self.rt.block_on(
            self.target
                .create(req.info(), &parent_path, name, mode, flags),
        ) {
            Ok(create) => {
                let (ino, generation) = self.inodes.add(Arc::new(parent_path.join(name)));
